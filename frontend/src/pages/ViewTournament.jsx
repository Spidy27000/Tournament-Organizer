...
const ViewTournament = () => {
  ...
  useEffect(() => {
    const creator = () => {
      if (storedTournament == tournamentId) {
        setIsCreator(true);
        console.log("ok");
      } else {
        setIsCreator(false);
  ...
  const handleEndMatch = () => {
    if (currentMatch < tournamentData.max_match) {
  ...
  return (
    <div className=" w-full p-9">
      ...
      {showConfetti && (
        <div className=" ">
        <Confetti width={windowSize.width - 10} height={windowSize.height - 10}/>
        </div>
      )}
      ...
      <div className="p-6">
        ...
        {isLastMatch && (
                  <div className=" pt-5 border-t-2 h-[35rem] flex flex-col" id="winnerScreen">
                  ...
                  <div className=" flex bg-[#f6f4f0] justify-center items-center w-[42%] rounded-md shadow-lg border-2 p-2">
                    ...
                    <div className=" w-[80%] flex justify-center items-center">
                  <h1 className=" font-extrabold text-[2.5rem] text-center">{score[2].teamName}</h1>
                  </div>
                  </div> 
                  </div>
                </div>
